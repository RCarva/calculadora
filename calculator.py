...
class Operações_Elementares(): #Vai cuidar de todas as operações mais simples e conversões
    ...
    def somatório(self): # Faz o uso de diferentes entradas
        ...
        if self.chk_soma==True:
            ...
            self.total=float(Display.entrada.get())
    ...
    def função(self): # Faz as operações se baseando nos botões com os comandos atrelados
        ...
        Display.persistencia(str(self.total))
        Display.persistencia("\n")
        #Display.close()
        #Display.open()
    ...
    def limpar(self): #Função que limpa o display
        ...
        self.valor=True
        Display.persistencia("\n")
        Display.persistencia("Limpar")
        Display.persistencia("\n")
        Display.persistencia(str(self.total)+self.op)
    ...
    def limpar_tudo(self): #Função que limpa as funções
        ...
        self.total=0
        Display.persistencia("\n")
        Display.persistencia("Limpar Tudo")
        Display.persistencia("\n")
...
#==========================================================================Conversões Rápidas==========================================================================
    ...
    def pi(self): #Converte π
        ...
        self.atual = math.pi
        self.display(self.atual)
        Display.persistencia(self.atual)
        Display.persistencia("\n")
    ...
    def raiz(self): #Faz a raiz do número no resultado
        self.resultado = False
        self.atual = math.sqrt(float(Display.entrada.get()))
        self.display(self.atual)
        Display.persistencia("^1/2 = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
  
    def cos(self): #Faz o Coseno do número no resultado
        self.resultado = False
        self.atual = math.cos(math.radians(float(Display.entrada.get())))
        self.display(self.atual)
        Display.persistencia(" cos = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")

    def tan(self): #Faz a Tangente do número no resultado
        self.resultado = False
        self.atual = math.tan(math.radians(float(Display.entrada.get())))
        self.display(self.atual)
        Display.persistencia(" tan = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")

    def seno(self): #Faz o Seno do número no resultado
        self.resultado = False
        self.atual = math.sin(math.radians(float(Display.entrada.get())))
        self.display(self.atual)
        Display.persistencia(" sen = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
  
    def log(self): #Faz o Log do número no resultado
        self.resultado = False
        self.atual = math.log(float(Display.entrada.get()))
        self.display(self.atual)
        Display.persistencia(" = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
    ...
    def exp(self): #Faz o Exponencial na base e do número no resultado
        self.resultado = False
        self.atual = math.exp(float(Display.entrada.get()))
        self.display(self.atual)
        Display.persistencia(" = ln(")
        Display.persistencia(str(self.atual))
        Display.persistencia(")\n")

    def quad(self): #Converte radianos em graus do número no resultado
        self.resultado = False
        self.atual = math.pow(float(Display.entrada.get()),2)
        self.display(self.atual)
        Display.persistencia("^2 = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
  
    def log2(self): #Faz o Log de base 2 do número no resultado
        self.resultado = False
        self.atual = math.log2(float(Display.entrada.get()))
        self.display(self.atual)
        Display.persistencia(" = 2^")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
    ...
    def log10(self): #Faz o Log de base 10 do número no resultado
        self.resultado = False
        self.atual = math.log10(float(Display.entrada.get()))
        self.display(self.atual)
        Display.persistencia(" = 10^")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
  
    def log1p(self): #Faz o Log na base e de 1+ o número no resultado
        self.resultado = False
        self.atual = math.log1p(float(Display.entrada.get()))
        self.display(self.atual)
        Display.persistencia(" = ")
        Display.persistencia(str(self.atual))
        Display.persistencia("\n")
...
class Display(): #Traduz as letras e símbolos e grava no arquivo de histórico
    ...
    def persistencia(dado):  #Abre um arquivo txt e grava o dado no arquivo
        with open(Display.caminho+"/Log da Calculadora.txt", "a") as log:
            dado=str(dado)
            log.writelines(dado)

    entrada = Entry(aba1, font = ("arial",20,"bold"), bg="white", bd=30, width=45,justify=RIGHT)
    entrada.grid(row = 0, column=0, columnspan=7,pady=1)
    entrada.insert(0,"0")
#=====================================================Limites=============================================================
...
class Lim():
    def __init__(self):
        ...
        self.equacao = sympify(Display.entrada.get())
        self.limite = 0
        self.derivada = 0

    def set_a(self): # define onde o limite vai ser calculado
        self.a = Display.entrada.get()
        print(self.a)
        

    #def monta_eq(self): # monta a equacao a partir da lista de entradas que o usuario colocou na calculadora
        #self.equacao += Display.entrada.get()
        #self.equacao = sympify(self.equacao)

    def calcula_lim(self): # calcula o limite)
        self.limite = limit(self.equacao, self.x, self.a)
        return self.limite

    def calcula_lim_direita(self): # calcula o limite a direita
        self.limite = limit(self.equacao, self.x, self.a, '+')
        return self.limite

    def calcula_lim_esquerda(self): # calcula o limite a esquerda
        self.limite = limit(self.equacao, self.x, self.a, '-')
        return self.limite
...
btnexpo = Button(aba1, width=6, height=2, font=("arial", 20, "bold"),fg="white", bd=4, text = "exp",command= lambda: valor_adicionado.exp, bg="grey58").grid(row = 5, column=5,pady=1)

btnx = Button(aba1, width=6, height=2, font=("arial", 20, "bold"),fg="white", bd=4, text = "a",command= limites.set_a(), bg="grey58").grid(row = 1, column=6,pady=1)

btnparent1 = Button(aba1, width=6, height=2, font=("arial", 20, "bold"),fg="white", bd=4, text = "(", bg="grey58").grid(row = 2, column=6,pady=1)

btnparent2 = Button(aba1, width=6, height=2, font=("arial", 20, "bold"),fg="white", bd=4, text = ")", bg="grey58").grid(row = 3, column=6,pady=1)

btnderiv = Button(aba1, width=6, height=2, font=("arial", 20, "bold"),fg="white", bd=4, text = "dy/dx", bg="grey58").grid(row = 4, column=6,pady=1)

btnlim = Button(aba1, width=6, height=2, font=("arial", 20, "bold"),fg="white", bd=4, text = "lim",command= limites.calcula_lim(), bg="grey58").grid(row = 5, column=6,pady=1)
...
menubar.add_cascade(label = "Sair" , command=saida)#adiciona a opcao do usuario clicar em "sair"
